...
void* worker_thread_handler(void* c) 
{
	RequestChannel* channel = (RequestChannel*) c;
	Data request = Data("",0,0);

	for (;;) 
	{
		request = request_buffer->getData(); 
		if (request.req_con == "die")
		{
			cout << "Worker thread killed" << endl;
			break; //to exit the loop and function
		}

		string reply = channel->send_request(request.req_con);
		request.req_con = reply;
		if (request.person_id == 0) // Joe 
			response_Joe_stats->insertData(request);
		else if (request.person_id == 1) // Jane
			response_Jane_stats->insertData(request);
		else if (request.person_id == 2) // John
			response_John_stats->insertData(request);
...
int main(int argc, char * argv[]) 
{
  // set default argument values
	...
	else //the parent, which is successfully still alive, executes this code
	{
		...
		cout << "Creating worker threads... " << flush; 
		for (int i = 0; i < workers_amount; i++)
		{
			string reply = chan.send_request("newthread"); // starts new thread for worker
			RequestChannel* channel = new RequestChannel(reply, 
				RequestChannel::CLIENT_SIDE);
			pthread_create(&workers_thread[i], NULL, worker_thread_handler, channel);
		}
		cout << "done." << endl;
