...
static int workers_amount;
static string server_host_name;
static unsigned short port_num;

static int joe_sent_count = 0;
static int jane_sent_count = 0;
static int john_sent_count = 0;
...
void* event_handler_thread(void* c) 
{
  ...
  NetworkRequestChannel * channels[workers_amount];
  fd_set readset;
  fd_set writeset;
  int max = 0;
  int result;
  ...
  for(int i = 0; i < workers_amount; i++) 
  {
    ...
    channels[i] = new NetworkRequestChannel(server_host_name, port_num);
    persons[i] = -1;
  }
  ...
  while(true)
  {
	...
	for(int i = 0; i < workers_amount; i++) 
	{
  		if(channels[i]->read_sockfd() > max)
		{
			max = channels[i]->read_sockfd();
		}
		...
		//it is probably not
		//needed here. 
		FD_SET(channels[i]->read_sockfd(), &readset);
		FD_SET(channels[i]->read_sockfd(), &writeset);
	}
	...
	if(result) 
	{
		for(int i = 0; i < workers_amount; i++) 
		{
		if(FD_ISSET(channels[i]->read_sockfd(), &readset)) 
		{
			cout << "----START READ\n"; 
  ...
  cout << "loop entirely exited" << endl;
  usleep(100000);
...
int main(int argc, char * argv[]) 
{
  // set default argument values
	...
	//if (childpid == 0)
	//{
		//exec_unless_error = execl("dataserver", "dataserver", "-p", "10201", "-b", "5", 0);
